...
export default function CoursePage({ params }) {
  ...
  // Debug: 打印slug和所有lesson的url
  console.log('DEBUG: 当前slug:', slug)
  if (chapter.sections) {
    chapter.sections.forEach(section => {
      section.lessons.forEach(lesson => {
        console.log('DEBUG: lesson.url:', lesson.url, 'isPreview:', lesson.isPreview)
        // 允许url前面多一个/cn/aideveloper或无/cn/aideveloper
        const normalizedLessonUrl = lesson.url.replace(/^\/cn\/aideveloper\//, '')
        const normalizedSlug = slug.replace(/^cn\//, '').replace(/^aideveloper\//, '').replace(/^\//, '')
        if (
          lesson.url === slug ||
          lesson.url === '/' + slug ||
          normalizedLessonUrl === normalizedSlug ||
          ('/' + normalizedLessonUrl) === normalizedSlug
        ) {
  ...
  console.log('DEBUG: 匹配到的currentLesson:', currentLesson)
